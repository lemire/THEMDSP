...
MV_Vector_double operator*(const MV_ColMat_double &M, const MV_Vector_double &x) {
      unsigned int N = x.size();
	  if((N != M.dim(0)) || (N != M.dim(0))) {
		  std::cout << " We're assuming a square matrix! Idiot! " << std::endl;
		  exit(0);
	  }
      MV_Vector_double result(N);
	  for(int k = 0; k < N; ++k)
